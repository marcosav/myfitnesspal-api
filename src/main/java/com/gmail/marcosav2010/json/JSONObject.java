...
public class JSONObject {
    ...
    /**
     * It is sometimes more convenient and less ambiguous to have a <code>NULL</code> object than to use
     * Java's <code>null</code> value. <code>JSONObject.NULL.equals(null)</code> returns
    ...
    public JSONObject() {
        ...
        // implementations to rearrange thesir items for a faster element
     ...
     *
     * @param m A ordered map object that can be used to initialize the contents of the JSONObject.
     * @throws JSONException        If a value in the map is non-finite number.
     * @throws NullPointerException If a key in the map is <code>null</code>
     */
     ...
     * <p>
     * Methods that are <code>static</code>, return <code>void</code>, have parameters, or are "bridge"
     * methods, are ignored.
     * <p>
     ...
     * serialized into JSON. If both {@link JSONPropertyIgnore} and {@link JSONPropertyName} are defined
     * on the same method, a depth comparison is performed and the one closest to the concrete class
     * being serialized is used. If both annotations are at the same level, then the
    ...
    /**
     * Construct a JSONObject from an Object, using reflection to find the public members. The resulting
     * JSONObject's keys will be the strings from the names array, and the values will be the field
     * values associated with those keys in the object. If a key is not found or not visible, then it
    ...
    /**
     * Construct a JSONObject from a source JSON text string. This is the most commonly used JSONObject
     * constructor.
     *
    ...
    /**
     * Accumulate values under a key. It is similar to the put method except that if there is already an
     * object stored under the key then a JSONArray is stored under the key to hold all of the
    ...
    /**
     * Append values to the array under a key. If the key does not exist in the JSONObject, then the key
     * is put in the JSONObject with its value being a JSONArray containing the value parameter. If the
     * key was already associated with a JSONArray, then the value parameter is appended to it.
     *
     * @param key   A key string.
     * @param value An object to be accumulated under the key.
     * @return this.
    ...
    /**
     * Get the BigDecimal value associated with a key. If the value is float or double, the the
     * {@link BigDecimal#BigDecimal(double)} constructor will be used. See notes on the constructor for
     ...
     * @param key A key string.
     * @return The numeric value.
     * @throws JSONException if the key is not found or if the value is not a Number object and cannot be
    ...
    /**
     * Increment a property of a JSONObject. If there is no such property, create one with a value of 1
     * (Integer). If there is such a property, and if it is an Integer, Long, Double, Float, BigInteger,
     * or BigDecimal then add one to it. No overflow bounds checking is performed, so callers should
     * initialize the key prior to this call with an appropriate type that can handle the maximum
     * expected value.
     *
     * @param key A key string.
     * @return this.
     * @throws JSONException If there is already a property with this name that is not an Integer, Long, Double,
     *                       or Float.
     */
    ...
    /**
     * Get a set of keys of the JSONObject. Modifying this key Set will also modify the JSONObject. Use
     * with caution.
     *
     * @return A keySet.
     * @see Map#keySet()
    ...
    /**
     * Get a set of entries of the JSONObject. These are raw values and may not match what is returned
     ...
     * cannot be assigned to <code>clazz</code>
     */
    ...
    /**
     * Get an optional BigDecimal associated with a key, or the defaultValue if there is no such key or
     * if its value is not a number. If the value is a string, an attempt will be made to evaluate it as
     * a number. If the value is float or double, then the {@link BigDecimal#BigDecimal(double)}
    ...
    /**
     * Get an optional BigInteger associated with a key, or the defaultValue if there is no such key or
     * if its value is not a number. If the value is a string, an attempt will be made to evaluate it as
     * a number.
     *
     * @param key          A key string.
     ...
     * its value is not a number. If the value is a string, an attempt will be made to evaluate it as a
     * number.
     *
     * @param key          A key string.
     * @param defaultValue The default.
    ...
    /**
     * Get the optional double value associated with an index. NaN is returned if there is no value for
    ...
    /**
     * Get the optional double value associated with an index. The defaultValue is returned if there is
    ...
    /**
     * Get an optional int value associated with a key, or zero if there is no such key or if the value
    ...
    /**
     * Get an optional int value associated with a key, or the default if there is no such key or if the
    ...
    /**
     * Get an optional JSONObject associated with a key. It returns null if there is no such key, or if
     * its value is not a JSONObject.
     *
     * @param key A key string.
     * @return A JSONObject which is the value.
     */
    ...
    /**
     * Get an optional long value associated with a key, or zero if there is no such key or if the value
     * is not a number. If the value is a string, an attempt will be made to evaluate it as a number.
     *
     * @param key A key string.
     * @return An object which is the value.
     ...
     * the value is not a number. If the value is a string, an attempt will be made to evaluate it as a
     * number.
     *
     * @param key          A key string.
     * @param defaultValue The default.
    ...
    /**
     * Populates the internal map of the JSONObject with the bean properties. The bean can not be
     * recursive.
     *
     * @param bean the bean
    ...
    /**
     * Put a key/value pair in the JSONObject, but only if the key and the value are both non-null, and
     * only if there is not already a member with that name.
     *
    ...
    /**
     * Determine if two JSONObjects are similar. They must contain the same set of names which must be
    ...
    /**
     * Try to convert a string into a number, boolean, or null. If the string can't be converted, return
     * the string.
     *
    ...
    /**
     * Make a JSON text of an Object value. If the object has an value.toJSONString() method, then that
     * method will be used to produce the JSON text. The method is required to produce a strictly
     * conforming text. If the object does not contain a toJSONString method (which is the most common
     * case), then a text will be produced by other means. If the value is an array or Collection, then
     * a JSONArray will be made from it and its toJSONString method will be called. If the value is a
     * MAP, then a JSONObject will be made from it and its toJSONString method will be called.
    ...
    /**
     * Wrap an object, if necessary. If the object is <code>null</code>, return the NULL object. If it
     * is an array or collection, wrap it in a JSONArray. If it is a map, wrap it in a JSONObject. If it
     * is a standard property (Double, String, et al) then it is already wrapped. Otherwise, if it comes
    ...
    public static Object wrap(Object object) {
        try {
            ...
            if (object instanceof JSONObject || object instanceof JSONArray || NULL.equals(object) || object instanceof JSONString || object instanceof Byte
    ...
    /**
     * Write the contents of the JSONObject as JSON text to a writer. For compactness, no whitespace is
     * added.
     * <p>
     * <b> Warning: This method assumes that the data structure is acyclical. </b>
     *
